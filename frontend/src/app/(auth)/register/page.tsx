...
export default function RegisterPage() {
  ...
  const onSubmit = async (data: RegisterFormData) => {
    ...
    try {
      ...
      setAuth(tokens, user);
      toast.success("Welcome to HackLaunch AI! 🚀");
  ...
  return (
    <div className="min-h-screen bg-dark-500 flex items-center justify-center px-4 relative overflow-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none">
        <div className="absolute top-0 left-1/3 w-96 h-96 bg-brand-600/12 rounded-full blur-[100px]" />
        <div className="absolute bottom-0 right-1/3 w-80 h-80 bg-accent-600/10 rounded-full blur-[80px]" />
      </div>
      ...
      <motion.div
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="font-display font-bold text-3xl text-white mt-2">Create your account</h1>
          <p className="text-surface-500 mt-1">Start launching hackathons with AI</p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="glass-card rounded-2xl p-6 space-y-4">
          {/* Full Name */}
          <div>
            <label className="block text-sm text-surface-300 mb-1.5 font-medium">Full Name</label>
            <div className="relative">
              <User className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-surface-600" />
              <input {...register("full_name")} placeholder="Rahul Kumar"
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input {...register("password")} type={showPassword ? "text" : "password"} placeholder="Min 8 chars, 1 uppercase, 1 number"
          ...
          {/* Confirm Password */}
          <div>
            <label className="block text-sm text-surface-300 mb-1.5 font-medium">Confirm Password</label>
            <div className="relative">
              <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-surface-600" />
              <input {...register("confirm_password")} type="password" placeholder="Repeat your password"
                className="w-full bg-dark-300/60 border border-white/8 text-white placeholder-surface-600 rounded-xl py-3 pl-10 pr-4 text-sm focus:border-brand-500/60 focus:outline-none transition-colors" />
            </div>
